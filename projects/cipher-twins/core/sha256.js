// Portable SHA-256, shared by the game UI and the commitment layer.
//
// `crypto.subtle` only exists in a secure context — HTTPS or localhost — so on
// a plain-HTTP LAN preview opened from another computer it is undefined and the
// digest call throws. `sha256Hex` uses the platform digest when it is available
// and falls back to a small self-contained implementation otherwise, so a
// same-network playtest still resolves guesses.
